package com.ojassoft.vartalive.rtc;
...
public class AgoraEventHandler extends IRtcEngineEventHandler {
    ...
    public void onRemoteAudioStateChanged(int uid, int state, int reason, int elapsed) {
        String logData = "Uid=" + uid + " State=" + state + " Reason=" + reason + " Elapsed=" + elapsed;

        if(AstrosageKundliApplication.liveRemoteAudioState == null){
            AstrosageKundliApplication.liveRemoteAudioState = "";
        }
        String timestamp = String.valueOf(System.currentTimeMillis());
        String currentTime = getTimeByTimestamp(timestamp);
        AstrosageKundliApplication.liveRemoteAudioState = AstrosageKundliApplication.liveRemoteAudioState +currentTime+ " ("+logData+") ## ";
        //Log.e("RmtAudioState", AstrosageKundliApplication.liveRemoteAudioState);
    ...
    public void onLocalAudioStateChanged(int state, int error) {
        ...
        String timestamp = String.valueOf(System.currentTimeMillis());
        String currentTime = getTimeByTimestamp(timestamp);
        AstrosageKundliApplication.liveLocalAudioState = AstrosageKundliApplication.liveLocalAudioState +currentTime+ " ("+state+","+error+") ## ";
        //Log.e("AgoraAudioState", AstrosageKundliApplication.liveLocalAudioState);

        for (EventHandler handler : mHandler) {
            handler.onLocalAudioStateChanged(state, error);
        }
    }
    ...
    private String getTimeByTimestamp(String sTime) {
        String time = "";
        try {
            long timestamp = Long.valueOf(sTime);
            Calendar cal = Calendar.getInstance(Locale.ENGLISH);
            cal.setTimeInMillis(timestamp);
            time = android.text.format.DateFormat.format("hh:mm:ss a", cal).toString();
        } catch (Exception e) {
            e.printStackTrace();
        }
