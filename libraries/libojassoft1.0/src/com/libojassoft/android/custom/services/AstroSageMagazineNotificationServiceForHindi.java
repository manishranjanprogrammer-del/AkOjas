...
public class AstroSageMagazineNotificationServiceForHindi extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (AstroSageMagazineNotificationServiceForHindi._fileoperations == null) {
            _rssFeedItemIndex = 0;
            mNotificationManager = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
            AstroSageMagazineNotificationServiceForHindi._fileoperations = new CFileOperations();
            //ADDED BY BIJENDRA ON 9-AUG-13
            try {
                Bundle extras = intent.getExtras();
                if (extras != null) {
                    try {
                        CALLER_AD_ID = extras.getString(LibCGlobalVariables.CALLER_APP_AD_ID);
    ...
    public void onDestroy() {
        ...
        AstroSageMagazineNotificationServiceForHindi._fileoperations = null;
       ...
       /* try//MODIFIED BY BIJENDRA 08-JAN-2013-APPLY  {TRY CATCH}
        {
            ...
            List<CMessage> listMessage = new CXmlPullFeedParser(RSSFEEDOFCHOICE,this,false).parse();

            if (listMessage != null) {	*//*
                 * String publishedDate =
				 * listMessage.get(0).getDate().toString().trim();
				 *
				 * Date pubDate=new Date(publishedDate.trim()); String
				 * _blogId=String.valueOf(pubDate.getDate())+
				 * String.valueOf(pubDate.getMonth())+
				 * String.valueOf(pubDate.getYear()); _blogId
				 * +=String.valueOf(pubDate.getHours())+
				 * String.valueOf(pubDate.getMinutes());
				 *//*// DISABLED BY DEEPAK ON 16-12-2014
                ...
                if (!LibCGlobalVariables.MAGAZINE_BOLG_ID
                    ...
                    if (!isSameNotificationSendByOtherAppplication(_blogId
						...
						*//*notification.contentView = contentView;
                        ...
                        Bitmap icon = BitmapFactory.decodeResource(AstroSageMagazineNotificationServiceForHindi.this.getResources(), com.libojassoft.android.R.mipmap.icon);
                        //String CHANNEL_ID = "hindi_article_notification";
                        NotificationCompat.Builder notification = new NotificationCompat.Builder(AstroSageMagazineNotificationServiceForHindi.this)
                        ...
                        _fileoperations
                                .saveMagazineNotificationJSONObjectHindi(
        ...
        AstroSageMagazineNotificationServiceForHindi.this.stopSelf();
    }

    private PendingIntent getPendingIntent(List<CMessage> listMessage, int noticicationId) {


        //String blog_link=listMessage.get(0).getLink().toString()+"?"+LibCUtils.getNotificationCompaignURL(getApplicationContext());;
        //ADDED BY BIJENRA ON 6-JULY-13
        String blog_link = LibCGlobalVariables.ASTROSAGE_BLOG_HOME_PAGE_HINDI + "?" + LibCUtils.getNotificationCompaignURL(getApplicationContext());
        ;
        //END


        //Intent notificationIntent = new Intent(this,ActShowOjasSoftProducts.class);

        Intent notificationIntent = getNotificationIntent();//THIS IS ADDED BY BIJENDRA ON 28-AUG-13
        ...
        //notificationIntent.putExtra("BLOG_LINK_TO_SHOW", listMessage.get(0).getLink().toString());// CHANGED BY HUKUM ON 06-MAY-2013
        ...
        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            contentIntent = PendingIntent.getActivity(getApplicationContext(), noticicationId, notificationIntent, PendingIntent.FLAG_IMMUTABLE);
        } else {
            contentIntent = PendingIntent.getActivity(getApplicationContext(), noticicationId, notificationIntent, PendingIntent.FLAG_ONE_SHOT);
        }
    ...
    private boolean isSameNotificationSendByOtherAppplication(String newBlogId) {
        ...
        try {
            ...
            obj = LibCUtils.getSavedJsonMagazineDetailObjectHindi(AstroSageMagazineNotificationServiceForHindi.this);
            if (obj != null) {
