...
/*@author Ankit
 */
public class PanchakInputFragment extends Fragment implements DateTimePicker.DateWatcher, OnVolleyResultListener {
    ...
    public static PanchakInputFragment newInstance(String text) {
        PanchakInputFragment panchakInputFragment = new PanchakInputFragment();
        ...
        topAdImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                CUtils.googleAnalyticSendWitPlayServie(activity,
                        ...
                        CGlobalVariables.GOOGLE_ANALYTIC_SLOT_37_Add, null);
                CUtils.fcmAnalyticsEvents(CGlobalVariables.GOOGLE_ANALYTIC_SLOT_37_Add, CGlobalVariables.FIREBASE_EVENT_ITEM_CLICK, "");

                CUtils.createSession(activity, "S37");
    ...
    private void getData() {
        ...
        try {
            ...
            if (result != null && !result.equals("")) {
                ...
                topAdData = CUtils.getSlotData(adList, "37");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void setLayRef(View view) {
        try {
            ...
            textViewCurrent.setText(getActivity().getResources().getString(R.string.panchak)
                    + " - "
    ...
    private void checkCachedData(String cityId) {
        String url = CGlobalVariables.panchakApiUrl;
        ...
        if (entry != null && cityId.equalsIgnoreCase(previousCityId) && previousMonth == (beanDateTime.getMonth() + 1)) {    // cache data
            ...
            try {
                ...
                if (saveData != null && !saveData.isEmpty()) {
                    parseApiResponse(saveData);
                }
                if (panchakDataList != null) {
                    mRecyclerView.setAdapter(new CustomAdapterPanchaknBhadra(activity, panchakDataList, monthArray, weekArray));
                } else {
                    checkAndCallApi(cityId);
                }
     ...
     * @param cityId
     */
    private void checkAndCallApi(String cityId) {
     ...
     * get panchak detail
     */
    public void getPanchakDetails(final String cityId) {
        showProgressBar();
        final String url = CGlobalVariables.panchakApiUrl;
        new VolleyServerRequest(getActivity(), PanchakInputFragment.this, url, getParams(cityId));
    }

    /**
     * @return hash map
     */
    private HashMap<String, String> getParams(String cityId) {
    ...
    /* public void updateBirthPlace(BeanPlace beanPlace) {
         ...
         try {
             ...
             //com.google.analytics.tracking.android.//Log.e(ex.getMessage().toString());
         }
     }*/
    public void updateBirthPlace(BeanPlace beanPlace) {
        this.beanPlace = beanPlace;
        ...
        try {
            ...
            updateBirthDate(beanDateTime);
            panchang.updateLayout(7);//7 means Panchak
        } catch (Exception ex) {
    ...
    public void shareContentData(String packageName) {
      ...
      /* SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT);
       String dateForButton = dateFormat.format(calendar.getTime());*/
        ...
        CUtils.shareData(activity, whatsAppData, packageName, activity.getResources().getString(R.string.panchak));
    ...
    private String getDataToShare(String singleSpace, String doubleSpace, String tripleSpace) {
        ...
        stringBuilder.append(giveMeSpace(17));
        stringBuilder.append(getActivity().getResources().getString(R.string.panchak) + " - " + monthArray[beanDateTime.getMonth()] + doubleSpace);
        ...
        if (panchakDataList != null && panchakDataList.size() > 0) {
            for (int iterator = 0; iterator < panchakDataList.size(); iterator++) {
                stringBuilder.append(titleImage);
                //set start date
                stringBuilder.append((CUtils.getDateInFormate(((FestDataDetail) panchakDataList.get(iterator)).getFestStartDate(), weekArray, monthArray)));
                stringBuilder.append(giveMeSpace(1));
                //set start time
                stringBuilder.append(CUtils.getTimeInFormate(((FestDataDetail) panchakDataList.get(iterator)).getStarthr(), ((FestDataDetail) panchakDataList.get(iterator)).getStartmin()).toUpperCase());
                ...
                stringBuilder.append(giveMeSpace(2));
                //set end date
                stringBuilder.append((CUtils.getDateInFormate(((FestDataDetail) panchakDataList.get(iterator)).getFestEndDate(), weekArray, monthArray)));
                stringBuilder.append(giveMeSpace(1));
                //set end time
                stringBuilder.append(CUtils.getTimeInFormate(((FestDataDetail) panchakDataList.get(iterator)).getEndhr(), ((FestDataDetail) panchakDataList.get(iterator)).getEndmin()).toUpperCase());
    ...
    @Override
    public void onVolleySuccess(String response, Cache cache) {
        hideProgressBar();
