...
public class NumerologyNameFragment extends Fragment {

    public int LANGUAGE_CODE = CGlobalVariables.ENGLISH;
    Activity currentActivity;
    Context context;
    private View view;
    private TextView namankTV;
    private TextView nameNumberTV;
    private TextView nameMulankDescTV;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        view = inflater.inflate(R.layout.fragment_numrology_name, container, false);
        initViews();
        initListner();
        return view;
    }
    ...
    private void initViews() {
        if (currentActivity == null || view == null) return;
        LANGUAGE_CODE = ((AstrosageKundliApplication) currentActivity.getApplication()).getLanguageCode();
        namankTV = view.findViewById(R.id.namankTV);
        nameNumberTV = view.findViewById(R.id.nameNumberTV);
        nameMulankDescTV = view.findViewById(R.id.nameMulankDescTV);
        lblHeadingTV = view.findViewById(R.id.lblHeadingTV);

        nameNumberTV.setTypeface(((BaseInputActivity) currentActivity).mediumTypeface, Typeface.BOLD);
        namankTV.setTypeface(((BaseInputActivity) currentActivity).regularTypeface);
        nameMulankDescTV.setTypeface(((BaseInputActivity) currentActivity).regularTypeface);
        ...
        llCustomAdv.addView(CUtils.getCustomAdvertismentView(currentActivity, false, ((NumerologyCalculatorOutputActivity) currentActivity).regularTypeface, "AKNNA"));
        initAdClickListner();
        topAdData = ((NumerologyCalculatorOutputActivity) currentActivity).getAddData("44");
        ...
        if (numerologyOutputModel != null) {
            nameNumberTV.setText(numerologyOutputModel.getNameNumber());
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                namankTV.setText(Html.fromHtml(numerologyOutputModel.getNameNumberDesc(), Html.FROM_HTML_MODE_COMPACT));
                nameMulankDescTV.setText(Html.fromHtml(numerologyOutputModel.getNameNumberTextDesc(), Html.FROM_HTML_MODE_COMPACT));
            } else {
                namankTV.setText(Html.fromHtml(numerologyOutputModel.getNameNumberDesc()));
                nameMulankDescTV.setText(Html.fromHtml(numerologyOutputModel.getNameNumberTextDesc()));
            }
        }
    }
        ...
        topAdImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                CUtils.googleAnalyticSendWitPlayServie(currentActivity,
                        ...
                        CGlobalVariables.GOOGLE_ANALYTIC_SLOT_44_Add, null);
                CUtils.fcmAnalyticsEvents(CGlobalVariables.GOOGLE_ANALYTIC_SLOT_44_Add, CGlobalVariables.FIREBASE_EVENT_ITEM_CLICK, "");

                CUtils.createSession(currentActivity, "S44");
