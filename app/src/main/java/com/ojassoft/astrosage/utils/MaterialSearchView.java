...
import com.ojassoft.astrosage.customadapters.SearchAdapter;
...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
        ...
        try {
            ...
            if (a != null) {
                ...
                if (a.hasValue(R.styleable.MaterialSearchView_searchCloseIcon)) {
                    Drawable closeIcon = a.getDrawable(R.styleable.MaterialSearchView_searchCloseIcon);
                    closeIcon.setTint(getResources().getColor(R.color.black));
                    setCloseIcon(closeIcon);
                }

                if (a.hasValue(R.styleable.MaterialSearchView_searchBackIcon)) {
                    Drawable backIcon = a.getDrawable(R.styleable.MaterialSearchView_searchBackIcon);
                    backIcon.setTint(getResources().getColor(R.color.black));
                ...
                if (a.hasValue(R.styleable.MaterialSearchView_searchSuggestionIcon)) {
                    Drawable suggestionIcon = a.getDrawable(R.styleable.MaterialSearchView_searchSuggestionIcon);
                    suggestionIcon.setTint(getResources().getColor(R.color.black));
            ...
            //Log.e("MaterialSearchEx","Ex"+e.getMessage());
        }
    }
    ...
    private void initiateView() {
        try {
            ...
            /*LinearLayout.LayoutParams abc = (LinearLayout.LayoutParams) mSearchTopBar.getLayoutParams();
            abc.topMargin = 70;
            mSearchTopBar.setLayoutParams(abc);*/
    ...
    private void onVoiceClicked() {
        try {
            ...
            intent.putExtra(RecognizerIntent.EXTRA_PROMPT, getContext().getString(R.string.speak_item_name));    // user hint
            intent.putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_WEB_SEARCH);    // setting recognition model, optimized for short phrases – search queries
            intent.putExtra(RecognizerIntent.EXTRA_MAX_RESULTS, 1);
            //  intent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_MINIMUM_LENGTH_MILLIS, (long) 1000);
            intent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, (long) 300);

            // quantity of results we want to receive
            //    if (mContext instanceof Activity) {
            ActAstroShop.activity.startActivityForResult(intent, REQUEST_VOICE);
            // }
     ...
     */
    public void setSuggestions(Context context, ArrayList<String> suggestions) {
    ...
    public void setQuery(CharSequence query, boolean submit) {
        ...
        if (query != null) {
            if (query.toString().contains("###")){
                String[] s = query.toString().split("###");
                mSearchSrcTextView.setText(s[0]);
            } else {
                mSearchSrcTextView.setText(query);
            }
            try {
    ...
    public void setTintVisibility(boolean mode) {
        if (mode)
            mTintView.setVisibility(VISIBLE);
        else
            mTintView.setVisibility(INVISIBLE);
