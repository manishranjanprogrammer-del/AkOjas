...
/**
 * This class is used to get calculated values
 * for chart from astrosage server.
 *
 * @author Bijendra
 * @version 1.0.0
 * @date 9 may 2012
...
public class COnlineDataCalculation extends com.cslsoftware.enghoro.EngHoro implements ICalculationProxy {
    ...
    /**
     * This variable contain the URL to hit on server to get
     * calculated values for chart.
     */
    //private String webServiceURI="http://app.astrosage.com/freekphorary/chartxml.asp?";
    public COnlineDataCalculation() {

    }

    /**
     * This  constructor is  used to initialize values that will be used in different operations.
     *
     * @param objBirthDetail
     */
    public COnlineDataCalculation(BeanHoroPersonalInfo objBirthDetail) {
        _birthDetail = objBirthDetail;
    }

    public COnlineDataCalculation(CalculateKundli calculateKundli, Context context, BeanHoroPersonalInfo objBirthDetail) {
        _birthDetail = objBirthDetail;
    ...
    /**
     * This constructor is use to initialize this OnSuccessKundliCalculation interface
     * @param calculateKundli
     * @param context
     * @param objBirthDetail
     */
    public COnlineDataCalculation(OnSuccessKundliCalculation calculateKundli, Context context, BeanHoroPersonalInfo objBirthDetail) {
        _birthDetail = objBirthDetail;
        this.context = context;
     ...
     * @param chartid
     */
    public COnlineDataCalculation(BeanHoroPersonalInfo objBirthDetail, int indexAyan, long chartid)//
    {
        _birthDetail = objBirthDetail;
        _chartId = chartid;
    }
    ...
    /**
     * This function is used to get chart calculated values from server
     *
     * @return void
     */
    ...
    /**
     * This function is used to execute request on server to get
     * calculated out put data from server.
     *
     * @return void
     * @throws Exception
     */
...
//		String strReturn = CGlobalVariables.OUT_PUT_DATA;
        ...
        HttpPost hp = new HttpPost(CGlobalVariables.CALCULATION_WEB_SERVICE_URL);
        try {


            hp.setEntity(new UrlEncodedFormEntity(getPrepareNameValuePairForKundli()));
    ...
    /**
     * This function is used to extract data fetched from server after executing
     * request and fill in object.
     *
     * @param strData
     * @return void
     */
...
//		_planetDegree.setAyan(Double.valueOf(_dValues));
        ...
        // SAN for Relation like Delibiated, exalted, friendly
...
//		CGlobal.getCGlobalObject().setOutPanchangObject(_outOutPanchang);
    ...
    /**
     * This function return List of name-value pairs for chart detail to send the request to
     * server to get calculated chart data from server.
     *
    ...
    private List<NameValuePair> getPrepareNameValuePairForKundli() {
        ...
        nameValuePairs.add(new BasicNameValuePair("sec", String.valueOf(_birthDetail.getDateTime().getSecond()).trim()));
    ...
    private String getWebServiceURI() {
        ...
        sb.append(CGlobalVariables.CALCULATION_WEB_SERVICE_URL + "name=" + _name);
        //sb.append("https://www.astrosage.com/freekphorary/chartxml.asp?name='"+CGlobalVariables._PersonInfo.getName().trim().replace(' ','%' ));
        sb.append("&sex=" + _birthDetail.getGender().trim());
        sb.append("&day=" + String.valueOf(_birthDetail.getDateTime().getDay()).trim());
        sb.append("&month=" + String.valueOf(_birthDetail.getDateTime().getMonth() + 1).trim());
        sb.append("&year=" + String.valueOf(_birthDetail.getDateTime().getYear()).trim());
        sb.append("&hrs=" + String.valueOf(_birthDetail.getDateTime().getHour()).trim());
        sb.append("&min=" + String.valueOf(_birthDetail.getDateTime().getMin()).trim());
        sb.append("&sec=0&dst=" + String.valueOf(_birthDetail.getDST()).trim());
        sb.append("&place=" + _birthDetail.getPlace().getCityName().trim());
        sb.append("&LongDeg=" + _birthDetail.getPlace().getLongDeg().trim());
        sb.append("&LongMin=" + _birthDetail.getPlace().getLongMin().trim());
        sb.append("&LongEW=" + _birthDetail.getPlace().getLongDir().trim());
        sb.append("&LatDeg=" + _birthDetail.getPlace().getLatDeg().trim());
        sb.append("&LatMin=" + _birthDetail.getPlace().getLatMin().trim());
        sb.append("&LatNS=" + _birthDetail.getPlace().getLatDir().trim());

        sb.append("&timeZone=" + String.valueOf(_birthDetail.getPlace().getTimeZoneValue()));
        sb.append("&ayanamsa=" + String.valueOf(String.valueOf(_birthDetail.getAyanIndex()).trim()));
        sb.append("&ayanamsa=" + String.valueOf(String.valueOf(_birthDetail.getAyanIndex()).trim()));
        sb.append("&kphn=" + String.valueOf(_birthDetail.getHoraryNumber()));
        sb.append("&languageCode=" + CGlobalVariables.LANGUAGE_CODE);
    ...
    private void doActionOnClick(final ICalculationProxy _ICalculationProxy) {
        ...
        //Log.e("SAN", "CODC kundli xml url " + url);
        CacheRequest cacheRequest = new CacheRequest(0, url, new Response.Listener<NetworkResponse>() {
            ...
            public void onResponse(NetworkResponse response) {
                try {
                    ...
                    //Log.e("SAN", "CODC kundli xml responseStr " + responseStr);
                    initValuesForMyKundli(responseStr);
                    separateKundliData(_ICalculationProxy);
                    if(onSuccessKundliCalculation != null){
                        onSuccessKundliCalculation.displayKundli();
                    }else if(calculateKundli != null){
                        calculateKundli.displayKundli();
                        calculateKundli.dismissProgressbar();
                    }
                    isShowToast = false;
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            public void onErrorResponse(VolleyError error) {
                if(calculateKundli == null) return;
                calculateKundli.dismissProgressbar();
                if (isShowToast) {
                    if (error instanceof NoConnectionError) {
                        MyCustomToast mct2 = new MyCustomToast(
                                ...
                                ((Activity) context), calculateKundli.typeface);
    ...
    private String getParameterString() {
        ...
        paramStr =
                "name" + "=" + _name + "&" +
                        ...
                        "place" + "=" + _birthDetail.getPlace().getCityName().trim().replace(" ","_") + "&" +
   ...
   /* public String saveNotesOnserver(Context context, String userid, String password, String usercomment, String onlinechartid) {
        return saveNotes(context, userid, password, usercomment, onlinechartid);
    }*/

    /*private String saveNotes(Context context, String userid, String password, String usercomment, String onlinechartid) {
        HttpResponse response = null;
        String strReturn = "";
        BufferedReader in = null;
        HttpClient hc = CUtils.getNewHttpClient();
        HttpPost hp = new HttpPost(CGlobalVariables.SAVE_NOTES_URL);
        try {
            hp.setEntity(new UrlEncodedFormEntity(getNameValuePairForSaveNotes(context, userid, password, usercomment, onlinechartid),"UTF-8"));
            response = hc.execute(hp);
            in = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
            StringBuffer sb = new StringBuffer("");
            String data = "";
   ...
   /* private List<NameValuePair> getNameValuePairForSaveNotes(Context context, String userId, String pwd, String userComment, String onlineChartid) {
        ...
        nameValuePairs.add(new BasicNameValuePair("usercomment", userComment));
        nameValuePairs.add(new BasicNameValuePair("onlinechartid", onlineChartid));
        return nameValuePairs;
    }*/
