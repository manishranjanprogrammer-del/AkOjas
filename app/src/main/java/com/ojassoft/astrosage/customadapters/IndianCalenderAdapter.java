...
public class IndianCalenderAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public IndianCalenderAdapter(Context ctx, ArrayList<PurnimaFastData> monthDataDetail, ArrayList<DetailApiModel> arrayListObj) {
        context = ctx;
    ...
    public void refreshList(ArrayList<PurnimaFastData> monthDataDetail) {
        festList.clear();
        mainList.clear();
        for (int i = 0; i < monthDataDetail.size(); i++) {
            festList.add(monthDataDetail.get(i));
        }
        mainList = filterFestArrayList(festList);
