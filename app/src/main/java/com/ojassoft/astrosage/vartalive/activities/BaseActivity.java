...
import com.ojassoft.astrosage.vartalive.rtc.AgoraEventHandler;
...
import com.ojassoft.astrosage.vartalive.utils.WindowUtil;
...
public abstract class BaseActivity extends AppCompatActivity implements EventHandler {
    ...
    /**
     * Applies configured custom fonts after a live activity layout is attached to the window.
     */
    @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            mRtcEngine = RtcEngine.create(getApplicationContext(), getString(R.string.livestreaming_app_id), mHandler);
            initConfig();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static void applyEdgeToEdgeInsets(Activity activity) {
        try {
            ...
            WindowCompat.setDecorFitsSystemWindows(activity.getWindow(), false); // enable edge-to-edge
            ...
            ViewCompat.setOnApplyWindowInsetsListener(
                    ...
                    (v, insets) -> {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                            ...
                            // Fallback for API < 30
                            android.view.WindowInsets systemInsets = insets.toWindowInsets();
                            if (systemInsets != null) {
                                int bottomInset = systemInsets.getSystemWindowInsetBottom();

                                v.setPadding(
                                        systemInsets.getSystemWindowInsetLeft(),
                                        0, // ignore status bar
                                        systemInsets.getSystemWindowInsetRight(),
                                        bottomInset
                                );
                            }
                        }
